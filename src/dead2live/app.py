...
from .rig import CartoonRigDetector, Rig
...
class Engine:
    """Caches the rig/animator for the current portrait."""
    def __init__(self):
        self.animator: PuppetAnimator | None = None
        self.key = None
    ...
    def set_image(self, image_rgb: np.ndarray):
        ...
        if key != self.key:
            rig = CartoonRigDetector().detect(bgr)
            self.animator = PuppetAnimator(bgr, rig)
            self.key = key
...
def generate(image_rgb, instruction, use_llm=True):
    ...
    beats = " → ".join(
        ...
        for p in plans)
    info = f"**[{src}]** {len(plans)} 段 · {beats}"
    return out, info


def preview_rig(image_rgb):
    """Show the detected rig overlay so the user can sanity-check detection."""
    from .rig import draw_rig_overlay
    if image_rgb is None:
        image_rgb = _load_default_rgb()
    bgr = cv2.cvtColor(image_rgb, cv2.COLOR_RGB2BGR)
    rig = CartoonRigDetector().detect(bgr)
    return cv2.cvtColor(draw_rig_overlay(bgr, rig), cv2.COLOR_BGR2RGB)


def build_ui():
    import gradio as gr
    with gr.Blocks(title="dead2live · 图/文生数字人", theme=gr.themes.Soft()) as demo:
        gr.Markdown(
            "# 🎭 dead2live — 图/文生可操控数字人\n"
            "上传一张肖像（或用默认的莎士比亚头像），输入指令，"
            "让角色做出表情和动作。支持中文/英文。\n\n"
